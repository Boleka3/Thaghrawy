...
1. **Guardrails** (`guardrails.py`) block destructive shell patterns before execution.
2. **Mode-gating** — a `recon_only` engagement never registers the exploit server.
3. **Human-approval gate** — dangerous tool calls require an explicit human verdict at runtime (`DANGEROUS_COMMANDS_REQUIRE_CONFIRM`).
4. **Least privilege & no raw-credential storage.**
5. **MCP-risk awareness** — tool-poisoning and prompt-injection risks are contained by keeping the human in the loop.
...
**Supervisor:** Prof. Haitham S. Hamza — Information Technology Department.
